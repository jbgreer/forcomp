...
object Anagrams {
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    def sentenceOccurrencesAnagram(o: Occurrences): List[Sentence] = {
      if (o.isEmpty) List(Nil)
      else {
        val combos = combinations(o)
        for {
          c <- combos if dictionaryByOccurrences.contains(c)
          words <- dictionaryByOccurrences(c)
          s <- sentenceOccurrencesAnagram(subtract(o, c))
        } yield words :: s
      }
    }

    sentenceOccurrencesAnagram(sentenceOccurrences(sentence))
  }
}
